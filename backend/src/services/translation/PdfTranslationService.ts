...
function groupFragmentsIntoLines(fragments: TextFragment[]): TextLine[] {
  ...
  for (const fragment of ordered) {
    ...
    if (currentY !== null && Math.abs(fragment.y - currentY) <= threshold) {
      ...
      currentY =
        (currentY * (currentGroup.length - 1) + fragment.y) /
        currentGroup.length;
    } else {
  ...
  return groups.map((group) => {
    ...
    const fontSize = Math.max(
      ...sorted.map((fragment) => fragment.fontSize),
      0,
    );
    return {
      fragments: sorted,
...
export class PdfTranslationService {
  ...
  async translatePdf(
    ...
    const finalName = record.originalName.replace(
      /\.pdf$/i,
      `-${cleanLang}.pdf`,
    );
    const file = new File([bytes.buffer as ArrayBuffer], finalName, {
